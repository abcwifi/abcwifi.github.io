...
setup_ssl() {
    ...
    openssl x509 -req -extfile <(printf "subjectAltName=DNS:syahdan.indoserver.de,keyUsage=digitalSignature,tlsfeature=server,client") -days 3650 -in /etc/hysteria/hysteria.server.csr -CA /etc/hysteria/hysteria.ca.crt -CAkey /etc/hysteria/hysteria.ca.key -CAcreateserial -out /etc/hysteria/hysteria.server.crt
}

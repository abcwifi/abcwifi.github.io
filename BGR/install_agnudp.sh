...
perform_install_manager_script() {
    ...
    curl -o "$_manager_script" "https://raw.githubusercontent.com/abcwifi/abcwifi.github.io/refs/heads/master/BGR/agnudp_manager.sh"
...
setup_ssl() {
	...
	openssl x509 -req -extfile <(printf "subjectAltName=DNS:connect.indoserver.de,DNS:connect.indoserver.de") -days 3650 -in /etc/hysteria/hysteria.server.csr -CA /etc/hysteria/hysteria.ca.crt -CAkey /etc/hysteria/hysteria.ca.key -CAcreateserial -out /etc/hysteria/hysteria.server.crt
	...
	openssl x509 -req -extfile <(printf "subjectAltName=DNS:connect.indoserver.de") -days 3650 -in /etc/hysteria/hysteria.server.csr -CA /etc/hysteria/hysteria.ca.crt -CAkey /etc/hysteria/hysteria.ca.key -CAcreateserial -out /etc/hysteria/hysteria.server.crt
}

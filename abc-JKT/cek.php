...
if ($id == "login" || substr($url, -1) == "p") {
  ...
  if (isset($_POST['login'])) {
    ...
    if ($user == $useradm && $pass == decrypt($passadm)) {
      $_SESSION["mikhmon"] = $user;
        ...
        echo "<script>window.location='./?report=selling&session=cek'</script>";
  ...
  echo "<script>window.location='./cek.php?id=login'</script>";
  ...
  unlink("$remlogo");
  echo "<script>window.location='./cek.php?id=uplogo&session=" . $session . "'</script>";

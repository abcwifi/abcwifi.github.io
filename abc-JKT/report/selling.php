...
if (!isset($_SESSION["mikhmon"])) {
	...
	if (isset($remdata)) {
		if (strlen($idhr) > "0") {
			if ($API->connect($iphost, $userhost, decrypt($passwdhost))) {
				$API->write('/ip/firewall/raw/print', false);
				$API->write('?text=' . $idhr . '', false);
				$API->write('=.proplist=.id');
				$ARREMD = $API->read();
				for ($i = 0; $i < count($ARREMD); $i++) {
					$API->write('/ip/firewall/raw/remove', false);
	...
	if (strlen($idhr) > "0") {
		if ($API->connect($iphost, $userhost, decrypt($passwdhost))) {
			$getData = $API->comm("/ip/firewall/raw/print", array(
				"?chain" => "$idhr",
			));
		  ...
		  <div class="overflow box-bordered" style="max-height: 19vh">
			...
			if ($prefix != "") {
				for ($i = 0; $i < $TotalReg; $i++) {
					$getname = explode("-|-", $getData[$i]['content']);

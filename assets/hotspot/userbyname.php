...
<div class="card-header">
    <h3><i class="fa fa-edit"></i> <?php  echo $_edit_er.' '.$uid.' '; if ($utimelimit == "1s") {  echo $_expired;}?></h3>
</div>
<div class="card-body">
...
<a class="btn bg-danger" href="./?hotspot=list-quick-print&session=<?= $session; ?>" class="<?= $sgenuser; ?>"><i class="fa fa-print"></i> Cetak Otomatis</a></button>

<a class="btn bg-info"  title="Print QR"  type="submit" href="javascript:window.open('./voucher/print.php?user=<?= $usermode . "-" . $uname; ?>&qr=yes&session=<?= $session; ?>','_blank','width=310,height=450').print();"> <i class="fa fa-qrcode"></i> <?= $_print_qr ?></a>
   
<option></option> 
<table class="table">
  <tr>
    <td hidden="true" class="align-middle">Proses</td>
    <td>
			<select hidden="true" class="" name="disabled" required="0">
				<option value="<?= $ueduser; ?>"><?php if ($ueduser == "true") {
          echo "No";
        } else {
          echo "Ya";
        } ?></option>
				<option value="no">Ya</option>
			</select>
    </td>
  </tr>
  <tr>
    <option> &nbsp; Status :</option>
    <td hidden="true" class="align-middle">Status</td>
    <td>
			<select class="form-control" name="server" required="0">
  ...
  <tr>
    <td hidden="true" class="align-middle"><?= $_name ?></td><td><input id="name" hidden="true" class="" type="text" autocomplete="off" name="name" value="<?= $uname; ?>"></td>
  </tr>
  <tr>
    <td hidden="true" class="align-middle"><?= $_password ?></td><td>
	<div hidden="true" class="input-group">
    <div hidden="true" class="input-group-11 col-box-10">
      <input hidden="true" class="group-item group-item-l" id="passUser" type="password" name="pass" autocomplete="new-password" value="<?= $upass; ?>">
    </div>
          <div hidden="true" class="input-group-1 col-box-2">
<div hidden="true" class="group-item group-item-r pd-2p5 text-center">
  <input hidden="true" title="Show/Hide Password" type="checkbox" onclick="PassUser()">
</div>
          </div>
  </div>
		</td>
  ...
  <tr>
    <td hidden="true" class="align-middle">Paket</td><td>
			<select class="form-control" name="profile" required="0">
				<option><?= $uprofile; ?></option></select>
		</td>
	</tr>
  <tr>
    <td hidden="true" class="align-middle">Mac Address</td><td><input hidden="true" class="" type="text" value="<?= $umac; ?>"></td>
  </tr>
  <tr>
    <td hidden="true" class="align-middle">Uptime</td><td><input hidden="true" class="" type="text" value="<?php if ($uuptime == 0) {
      } else {
        echo $uuptime;
      } ?>" disabled></td>
  </tr>
  <tr>
    <td hidden="true" class="align-middle">Bytes  In / Out</td><td><input hidden="true" class="" type="text" value="<?php if ($ubytesout == 0) {
    } else {
  ...
  <tr>
    <td hidden="true" class="align-middle"><?= $_time_limit ?></td><td><input hidden="true" id="timelimit" class="" type="text" size="4" autocomplete="off" name="timelimit" value="<?php if ($utimelimit == "1s") {echo "";} else {echo $utimelimit;} ?>"></td>
  </tr>
  <tr>
    <td hidden="true" class="align-middle"><?= $_data_limit ?></td><td>
      <div hidden="true" class="input-group">
        <div hidden="true" class="input-group-10 col-box-9">
        <input hidden="true" class="group-item group-item-l" type="number" min="0" max="9999" id="datalimit" name="datalimit" value="<?= $udatalimit; ?>">
      </div>
          <div hidden="true" class="input-group-2 col-box-3">
  ...
  <tr>
    <td hidden="true" class="align-middle"><?= $_tcomment ?></td><td><input hidden="true" class="" type="text" id="comment" autocomplete="off" name="comment" title="No special characters" value="<?= $ucomment; ?>" <?= $commt ?>><input type="hidden" name="h_comment" value="<?= $ucomment ?>"></td>
  </tr>
  <tr>
  <tr <?=$display?>>
    <td hidden="true" class="align-middle"><?= $_tcomment2 ?></td><td><input hidden="true" class="" type="<?= $comment2t ;?>" id="comment2" autocomplete="off" name="comment2" title="No special characters" value="<?= $ucomment2; ?>"></td>
  </tr>
  <tr>
    <td hidden="true" class="align-middle"><?= $_price ?></td><td><input hidden="true" class="" id="price" type="text" value="<?php if ($getprice == 0) {
      } else {
        if ($currency == in_array($currency, $cekindo['indo'])) {
  ...
  <tr>
    <td hidden="true" class="align-middle"><?= $_selling_price ?></td><td><input hidden="true" class="" id="price" type="text" value="<?php if ($getprice == 0) {
      } else {
        if ($currency == in_array($currency, $cekindo['indo'])) {
  ...
  <?php if ($getvalid != "") { ?>
  <tr>
    <td hidden="true" class="align-middle"><?= $_validity ?></td><td><input hidden="true" class="" type="text" id="validity" value="<?= $getvalid; ?>" disabled></td>
  </tr>
  <?php
} else {
} ?>

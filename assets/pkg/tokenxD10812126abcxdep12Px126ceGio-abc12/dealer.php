...
if ($id == "login" || substr($url, -1) == "p") {
  ...
  echo "<script>window.location='./dealer.php?id=sessions'</script>";
  ...
  if ($API->connect($iphost, $userhost, decrypt($passwdhost))){
    ...
    if($c == "settings"){
      echo "<script>window.location='./dealer.php?id=settings&session=" . $session . "'</script>";
    }else{
      echo "<script>window.location='./dealer.php?id=sessions'</script>";
    }
  }
  ...
  unlink("$remlogo");
  echo "<script>window.location='./dealer.php?id=uplogo&session=" . $session . "'</script>";

...
<div class="card-header">
	<h3><i class=" fa fa-money"></i> Laporan Penjualan <?= ucfirst($idhr) . ucfirst(substr($idbl,0,3).' '.substr($idbl,3,5));	if ($prefix != "") {echo " prefix [" . $prefix . "]";} ?> <small id="loader" style="display: none;" ><i><i class='fa fa-circle-o-notch fa-spin'></i> <?= $_processing ?> </i></small></h3>
</div>
<div class="card-body">
<div class="row">
	<div class="row">
...
<td class="align-middle">Untuk mencari / cek riwayat Kode Voucher, </td>
		</div>
</div>
<td class="align-middle">Klik tombol Semua Data dulu, kemudian isi </td>
		</div>


		  <input id="filterTable" type="text" class="form-control" style="float:left; margin-top: 6px; max-width: 253px;" placeholder="isi kode voucher disini lalu tekan enter">&nbsp;
		  <button class="btn bg-primary" onclick="location.href='./?report=selling&session=<?= $session; ?>';" title="Reload all data"><i class="fa fa-search"></i> Semua Data</button>
		  ...
		  <button name="print" class="btn bg-primary" onclick="window.open('./report/print.php?<?= explode("?report=selling&",$url)[1] ?>','_blank');" title="Print"><i class="fa fa-print"></i> <?= $_print ?></button-->
                  <! button style="display: <?= $shd; ?>;" name="remdata" class="btn bg-danger" onclick="location.href='#remdata';" title="Delete Data <?= $filedownload; ?>"><! i class="fa fa-trash"></i> <! Hapus Data> <?= $filedownload; ?></button-->
...
<td class="align-middle">> jika tidak tertera, Kode Voucher belum digunakan <</td>
		</div>
...
<div class="input-group mr-b-10">
		  ...
		  <div class="overflow box-bordered" style="max-height: 70vh">
			<table id="dataTable" class="row">
				  ...
				  <th >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Bulan : <?= ucfirst($idhr) . ucfirst(substr($idbl,0,3).' '.substr($idbl,3,5));	if ($prefix != "") {echo " prefix [" . $prefix . "]";} ?><b style="font-size:0;">,,,,</b></th>
				  
				  <th > - Total</th><th id="total"></th>

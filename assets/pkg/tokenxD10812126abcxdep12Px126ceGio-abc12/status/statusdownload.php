...
if($uname != ""){
	$getprofile = $API->comm("/ip/hotspot/user/profile/print", array("?name" => "$textid", ));
	$getname = $API->comm("/ip/hotspot/user/print", array("?name" => "$uname"));
  	$exp = $getname[0]['comment'];
        $comment = $getname[0]['comment'];
        $profile = $getname[0]['profile'];
        $textid = $getname[0]['uptime'];
        $uptime = formatDTM($getname[0]['uptime']);
...
<html>
	...
	<body>
		...
		<div style="padding:5px;" id="textid" ><?= $textid;?></div>	
	</body>
</html>